...
if (empty($token)) {
    ...
    // Find tenant by token, ensuring it has not expired
    $stmt = $conn->prepare("SELECT id FROM tenants WHERE reset_token = ? AND reset_token_expires_at > NOW()");
    ...
    if ($result->num_rows > 0) {
        ...
        if ($_SERVER["REQUEST_METHOD"] === "POST") {
            ...
            if ($password !== $password_confirm) {
                ...
                // Passwords match and meet criteria, update it
                $hashed_password = password_hash($password, PASSWORD_DEFAULT);

                // Update password and clear the reset token
                $update_stmt = $conn->prepare("UPDATE tenants SET password = ?, reset_token = NULL, reset_token_expires_at = NULL WHERE id = ?");
