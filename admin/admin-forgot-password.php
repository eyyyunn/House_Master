...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        $expires = date("Y-m-d H:i:s", time() + 3600); // 1 hour before token expires

        
        $update_stmt = $conn->prepare("UPDATE admins SET reset_token = ?, reset_token_expires_at = ? WHERE id = ?");
        $update_stmt->bind_param("ssi", $token, $expires, $admin_id);
        $update_stmt->execute();

        
        $reset_link = "http://" . $_SERVER['HTTP_HOST'] . dirname($_SERVER['PHP_SELF']) . "/admin-reset-password.php?token=" . $token;
